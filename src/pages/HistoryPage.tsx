...
export function HistoryPage({ refreshKey, onToast }: HistoryPageProps) {
  ...
  const handleFav = async (id: string, fav: boolean) => {
    ...
    try { await toggleFavorite(id, fav); } catch {
      onToast("操作失敗");
      setRecords((prev) => prev.map((r) => (r.id === id ? { ...r, is_favorite: !fav } : r)));
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteTranslation(id); onToast("已刪除"); } catch {
      onToast("刪除失敗");
      setRecords(prev);
    }
  };
  ...
  return (
    <div className="page history-page">
      <h1 className="history-title">翻譯歷史</h1>
      <div className="history-filter">
        <button className={`filter-chip ${filter === "all" ? "active" : ""}`} onClick={() => setFilter("all")}>全部</button>
        <button className={`filter-chip ${filter === "favorite" ? "active" : ""}`} onClick={() => setFilter("favorite")}>★ 收藏</button>
      </div>

      {loading ? (
        <div className="history-list">
          {[1, 2, 3].map((i) => (
            <div key={i} className="history-item" style={{ height: 100 }}>
        ...
        <div className="empty-state">
          ...
          <div className="empty-desc">{filter === "favorite" ? "點擊歷史記錄中的星星圖示即可收藏" : "開始翻譯後，記錄會自動儲存在這裡"}</div>
        </div>
      ) : (
        <div className="history-list">
          {filtered.map((r, i) => (
            <div key={r.id} className="history-item anim-up" style={{ animationDelay: `${Math.min(i * 0.04, 0.3)}s` }}>
              <div className="history-item-header">
                <div className="history-lang-pair">
                  <span>{getLanguage(r.source_lang).flag}</span>
                  <span>{getLanguage(r.source_lang).nativeName}</span>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ width: 12, height: 12 }}>
                    <path d="M5 12h14M12 5l7 7-7 7" />
                  </svg>
                  <span>{getLanguage(r.target_lang).flag}</span>
                  <span>{getLanguage(r.target_lang).nativeName}</span>
                  <span className="history-context-tag">{getContextMode(r.context_mode).icon} {getContextMode(r.context_mode).name}</span>
                </div>
                <div style={{ display: "flex", gap: 4 }}>
                  <button className={`history-fav-btn ${r.is_favorite ? "active" : ""}`} onClick={() => handleFav(r.id, !r.is_favorite)} aria-label="收藏">
